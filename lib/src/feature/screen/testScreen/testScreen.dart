...
class _TestscreenState extends State<Testscreen> {
  ...
  Future<void> fetchFrames() async {
    try {
      final response = await http.get(
          Uri.parse(BaseurlForBackend.getFrames)); // Use your API endpoint here
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Failed to load frames')));
      }
    } catch (error) {
      ScaffoldMessenger.of(context)
  ...
  Widget build(BuildContext context) {
    print('ll${MediaQuery.of(context).size.width}');
    // print('ll${MediaQuery.of(context).size.height}');
    return Scaffold(
      appBar: AppBar(
